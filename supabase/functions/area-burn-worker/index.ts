...
async function waitForConfirmation(
  ...
  while (
    ...
    if (status) {
      if (status.err) {
        throw new Error(
          `Solana burn transaction failed: ${
...
async function reconcileBurns(
  ...
  for (const row of rows) {
    ...
    try {
       ...
       * IMPORTANT:
       *
       * null does NOT mean failed.
       * Never create another burn while
       * an existing signature is unresolved.
       */
      if (!transaction) {
        results.push({
          burnId: row.id,
       ...
       * and definitively failed.
       *
       * Only now is retrying safe.
       */
      const {
        data: failureResult,
        error: failureError,
...
async function processNextBurn(
  ...
  try {
    ...
    /*
     * Critical guard against the historical
     * decimals bug:
     *
     * 1 AREA MUST be 1,000,000,000 raw units.
     */
    if (
      rawAmount !==
      expectedRawAmount
    ...
    /*
     * ----------------------------------------
     * 9. IDEMPOTENCY BARRIER
     *
     * Signature is recorded BEFORE broadcast.
     * ----------------------------------------
     */
    ...
    /*
     * ----------------------------------------
     * 10. Broadcast exact signed transaction
     * ----------------------------------------
     */
    ...
    /*
     * If no signature exists yet,
     * no burn can have been broadcast
     * by this execution.
     *
     * Safe to requeue.
     *
     * Once a signature exists,
...
Deno.serve(
  async (req: Request) => {
    try {
      ...
      /*
       * Existing signed burns are always
       * reconciled BEFORE claiming new work.
       */
      ...
      /*
       * At most ONE new burn per invocation.
       */
      const burn =
